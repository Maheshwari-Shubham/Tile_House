...
export default function ProductsPage() {
  ...
  const filtered = allProducts.filter(p => {
    const matchCompany = activeCompany === 'All' || p.company === activeCompany;
    ...
    const matchSearch = !search || p.name.toLowerCase().includes(q) || p.dimensions.toLowerCase().includes(q);
